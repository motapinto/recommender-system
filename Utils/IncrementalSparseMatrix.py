...
class IncrementalSparseMatrix_ListBased(object):
    def __init__(self, auto_create_col_mapper = False, auto_create_row_mapper = False, n_rows = None, n_cols = None):
        ...
        if self._auto_create_column_mapper: self._column_original_ID_to_index = {}
        if self._auto_create_row_mapper: self._row_original_ID_to_index = {}
    ...
    def get_column_token_to_id_mapper(self):
        if self._auto_create_column_mapper: return self._column_original_ID_to_index.copy()
        
        dummy_column_original_ID_to_index = {}
        for col in range(self._n_cols): dummy_column_original_ID_to_index[col] = col

        return dummy_column_original_ID_to_index

    def get_row_token_to_id_mapper(self):
        if self._auto_create_row_mapper: return self._row_original_ID_to_index.copy()
...
class IncrementalSparseMatrix_FilterIDs(IncrementalSparseMatrix):
    ...
    def __init__(self, preinitialized_col_mapper = None, preinitialized_row_mapper = None,
        ...
        assert on_new_col in ['add', 'ignore'], f'IncrementalSparseMatrix: if_new_col value not recognized, allowed values are \'add\', \'ignore\', provided was {on_new_col}'.format(on_new_col)
        assert on_new_row in ['add', 'ignore'], f'IncrementalSparseMatrix: if_new_row value not recognized, allowed values are \'add\', \'ignore\', provided was {on_new_row}'.format(on_new_row)
