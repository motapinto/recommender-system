...
class Evaluator(object):
    ...
    def __init__(
        self, URM_test_list, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        if not np.all(users_to_evaluate_mask):
            self._print('Ignoring {} ({:4.1f}%) Users that have less than {} test interactions'.format(
                np.sum(users_to_evaluate_mask), 100*np.sum(np.logical_not(users_to_evaluate_mask))/len(users_to_evaluate_mask), min_ratings_per_user))
    ...
    def _print(self, string):
        if self.verbose: print('{}: {}'.format(self.EVALUATOR_NAME, string))
...
class EvaluatorNegativeItemSample(Evaluator):
    ...
    def __init__(
        self, URM_test_list, URM_test_negative, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        diversity_object = None,
        ignore_items = None,
        ignore_users = None):
        '''
