...
def assert_disjoint_matrices(URM_list):
    ...
    for URM in URM_list:
        ...
        if URM_implicit_global is None: URM_implicit_global = URM_implicit
        else: URM_implicit_global += URM_implicit
...
def assert_URM_ICM_mapper_consistency(
    URM_DICT, user_original_ID_to_index, item_original_ID_to_index,
    ...
    if ICM_MAPPER_DICT is not None:
        ...
        for ICM_name, ICM_object in ICM_DICT.items():
            assert ICM_name in ICM_MAPPER_DICT, print_preamble + f'No mapper is available for ICM {ICM_name}'
    ...
    if UCM_MAPPER_DICT is not None:
        ...
        for UCM_name, UCM_object in UCM_DICT.items():
            assert UCM_name in UCM_MAPPER_DICT, print_preamble + f'No mapper is available for UCM {UCM_name}'
