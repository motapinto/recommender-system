import time, sys
import numpy as np
from Recommenders.Base.BaseTempFolder import BaseTempFolder
    ...
    def _train_with_early_stopping(
        ...
        if not convergence:
            ...
            if evaluator_object is not None and self.best_validation_metric is not None:
                print(f'{algorithm_name}: Terminating at epoch {epochs_current}. Best value for {validation_metric} at epoch {self.epochs_best} is {self.best_validation_metric}. Elapsed time {new_time_value} {new_time_unit}')          
            else:
                print(f'{algorithm_name}: Terminating at epoch {epochs_current}. Elapsed time {new_time_value} {new_time_unit}')
