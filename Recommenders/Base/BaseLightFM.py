...
class BaseLightFM(Base, Incremental_Training_Early_Stopping):
    '''
    ...
    def _compute_item_score(self, user_id_array, items_to_compute = None):
        ...
        for user_index, user_id in enumerate(user_id_array):
            item_scores[user_index,items_to_compute] = self.lightFM_model.predict(
                int(user_id), items_to_compute,
                item_features=self.ICM_train,
    ...
    def fit(self, epochs = 300, loss = 'bpr', sgd_mode = 'adagrad', n_components = 10,
            item_alpha = 0.0, user_alpha = 0.0, learning_rate = 0.05, num_threads = 4, **earlystopping_kwargs):
        ...
        # self.lightFM_model = self.lightFM_model.fit(
        #     self.URM_train,
        #     item_features = self.ICM_train,
        #     user_features = self.UCM_train,
        #     epochs=epochs,
    ...
    def _run_epoch(self, num_epoch):
        self.lightFM_model = self.lightFM_model.fit_partial(
            self.URM_train,
            item_features = self.ICM_train,
            user_features = self.UCM_train,
            epochs = 1,
